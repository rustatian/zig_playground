...
pub const struct_ggml_tensor = extern struct {
    ...
    src: [4][*c]struct_ggml_tensor = @import("std").mem.zeroes([4][*c]struct_ggml_tensor),
};
pub const struct_ggml_context = opaque {};
pub const struct_simple_model = extern struct {
    a: [*c]struct_ggml_tensor = @import("std").mem.zeroes([*c]struct_ggml_tensor),
    b: [*c]struct_ggml_tensor = @import("std").mem.zeroes([*c]struct_ggml_tensor),
    ctx: ?*struct_ggml_context = @import("std").mem.zeroes(?*struct_ggml_context),
};
pub export fn load_model(arg_model: [*c]struct_simple_model) void {
    var model = arg_model;
    _ = &model;
    model.*.a = @as([*c]struct_ggml_tensor, @ptrFromInt(@as(c_int, 4660)));
    return;
}
pub export fn main() c_int {
    var model: struct_simple_model = undefined;
    _ = &model;
    load_model(&model);
    return 0;
}
...
pub const simple_model = struct_simple_model;
